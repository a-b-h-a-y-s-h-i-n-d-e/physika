...
physika_print(a)
cos_wave = torch.stack([(torch.cos((i * 0.5)) if isinstance((i * 0.5), torch.Tensor) else math.cos((i * 0.5))) for _fi_i in range(int(6)) for i in [torch.tensor(float(_fi_i))]])
