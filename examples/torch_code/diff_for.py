...
# === Functions ===
def sum_for_expr(s):
    return (torch.sum(torch.stack([(s * i) for _fi_i in range(int(4)) for i in [torch.tensor(float(_fi_i))]])) if isinstance(torch.stack([(s * i) for _fi_i in range(int(4)) for i in [torch.tensor(float(_fi_i))]]), torch.Tensor) else sum(torch.stack([(s * i) for _fi_i in range(int(4)) for i in [torch.tensor(float(_fi_i))]])))
...
def matmul_scale(s):
    ...
    return (torch.sum(C3) if isinstance(C3, torch.Tensor) else sum(C3))
...
def cos_freqs(x):
    return torch.stack([(torch.cos((x * (i + 1))) if isinstance((x * (i + 1)), torch.Tensor) else math.cos((x * (i + 1)))) for _fi_i in range(int(4)) for i in [torch.tensor(float(_fi_i))]])
