...
class TestExprArray:
    ...
    def test_3D_nesting(self):
        ...
        # t should be ℝ[2, 2, 2]
        assert t == TTensor(((2, "invariant"), (2, "invariant"), (2, "invariant")))
    ...
    def test_variable_elements(self):
        ...
        # a : R[4]
        # b: R[4]
        ctx = make_ctx(env={"a": TTensor(((4, "invariant"),)),
    ...
    def test_substitution(self):
        """
        Unification of a TVar element against a concrete type produces a binding.
        """
        alpha = TVar("α0")
    ...
    def test_mixed_type_error(self):
        ...
        errors = []
        ctx = make_ctx(
            env={"v": TTensor(((3, "invariant"),)), "w": TTensor(((5, "invariant"),))},
            errors=errors,
        )
        # ℝ[3] and ℝ[5] cannot be unified, should add an error
        expr_array(("array", [("var", "v"), ("var", "w")]), ctx)
        assert len(errors) == 1
        assert 'Inconsistent array element types at index 1: Dimension mismatch: 3 ≠ 5' == errors[0]

        # no error for compatible shapes
        errors =[]
        ctx_no_error= make_ctx(env={"a": TTensor(((3, "invariant"),)),
...
class TestExprIndex:
    ...
    def test_index(self):
        ...
        ctx = make_ctx(env={"A": TTensor(((3, "invariant"), (4, "invariant")))})
        t, _ = expr_index(("index", "A", ("num", 0)), ctx)
        assert t == ("tensor", [(4, "invariant")])

   
        #Indexing a 3D tensor along dim-0 produces a 2D matrix
        ctx = make_ctx(env={"T": TTensor(((2, "invariant"), (3, "invariant"), (4, "invariant")))})
        t, _ = expr_index(("index", "T", ("num", 0)), ctx)
        assert t == ("tensor", [(3, "invariant"), (4, "invariant")])

    def test_unknown_variable(self):
        """
        Unknown array name returns (None, ctx.s) without error, error is raised at compile time.
        """
        errors = []
        ctx = make_ctx(errors=errors)
    ...
    def test_substitution_gains_binding(self):
        """
        Indexing with a TDim index variable unifies it against the leading dimension.
        ...
        s = Substitution()
        ctx = make_ctx(
            env={"v": TTensor(((5, "invariant"),)),
                  "i": TDim("δ0")}, # "δ0" is the size to range over, determined by unification 
            s=s,
        )
...
class TestExprIndexN:
    ...
    def test_3d_index(self):
        ...
        ctx = make_ctx(env={"T": TTensor(((2, "invariant"), (3, "invariant"), (4, "invariant")))})
        t, _ = expr_indexN(("indexN", "T", [("num", 0), ("num", 1), ("num", 2)]), ctx)
        assert t == T_REAL
    ...
    def test_unknown_variable_returns_none(self):
        ...
        errors = []
        ctx = make_ctx(errors=errors)
        t, _ = expr_indexN(("indexN", "missing", [("num", 0), ("num", 1)]), ctx)
        assert t is None
        assert errors == []
    ...
    def test_over_indexed(self):
        ...
        t, _ = expr_indexN(("indexN", "v", [("num", 0), ("num", 1), ("num", 2)]), ctx)
        assert t is None
        assert len(errors) == 1
    ...
    def test_variable_indices(self):
        ...
        """
        # A : ℝ[3, 4][i][j] -> ℝ
        ctx = make_ctx(env={"A": TTensor(((3, "invariant"),
                                          (4, "invariant"))),
                            "i": T_REAL,
                            "j": T_REAL})
...
class TestExprChainIndex:
    ...
    def test_tensor_chain(self):
        ...
        ctx = make_ctx(env={"T": TTensor(((2, "invariant"), (3, "invariant"), (4, "invariant")))})
        inner = ("index", "T", ("num", 0))   # T[0] -> ('tensor', [(3,'invariant'),(4,'invariant')])
        t, _ = expr_chain_index(("chain_index", inner), ctx)
        assert t == TTensor(((4, "invariant"),))
    ...
    def test_1d_chain_is_overindex(self):
        ...
        assert t is None
        assert len(errors) == 1
        assert "Chain index applied to a scalar: cannot index a scalar" in errors[0]
        assert isinstance(s_out, Substitution)
    ...
    def test_chain_on_scalar(self):
        ...
        errors = []
        ctx = make_ctx(env={"x": T_REAL}, errors=errors)
        # ("var", "x") infers T_REAL; chain_index then tries to peel a dim from scalar
        inner = ("var", "x")
        t, _ = expr_chain_index(("chain_index", inner), ctx)
        assert t is None
        assert len(errors) == 1
    ...
    def test_chain_on_failed_inner_propagates_none(self):
        """When inner inference returns None (e.g. unknown var), no second error is added."""
        errors = []
        ctx = make_ctx(errors=errors)
...
class TestExprSlice:
    ...
    def test_both_dynamic_gives_symbolic_dim(self):
        ...
        """
        ctx = make_ctx(env={"v": TTensor(((6, "invariant"),)), "a": T_REAL, "b": T_REAL})
    ...
    def test_dynamic_bounds_2d(self):
        ...
        ctx = make_ctx(env={"A": TTensor(((3, "invariant"), (4, "invariant"))), "i": T_REAL})
    ...
    def test_unknown_variable_returns_none(self):
        ...
        ctx = make_ctx()
        t, _ = expr_slice(("slice", "missing", ("num", 0.0), ("num", 3.0)), ctx)
        assert t is None
    ...
    def test_start_out_of_bounds_reports_error(self):
        ...
        assert t is None
        assert len(errors) == 1
        assert errors[0] == "Slice start 4 is out of bounds for 'v' with leading dimension 4"
    ...
    def test_end_out_of_bounds_reports_error(self):
        ...
        assert t is None
        assert len(errors) == 1
        assert errors[0] == "Slice end 5 is out of bounds for 'v' with leading dimension 4"
...
class TestExprAddSub:
    ...
    def test_vector(self):
        ...
        # ℝ[4] - ℝ[4] → ℝ[4]
        ctx = make_ctx(env={"a": TTensor(((4, "invariant"),)), "b": TTensor(((4, "invariant"),))})
    ...
    def test_matrix_and_matrix(self):
        ...
        errors = []
        ctx = make_ctx(env={"A": TTensor(((3, "invariant"), (2, "invariant"))),
                            "B": TTensor(((2, "invariant"), (3, "invariant")))},
                        errors=errors)
        t, _ = expr_add_sub(("add", ("var", "A"), ("var", "B")), ctx)
        assert len(errors) == 1
    ...
    def test_shape_mismatch_reports_error(self):
        ...
        errors = []
        ctx = make_ctx(env={"x": TTensor(((3, "invariant"),)),
                            "y": TTensor(((5, "invariant"),))}, errors=errors)
...
class TestInferExpr:
    ...
    def test_var_node(self):
        ...
        t, _ = infer_expr(("var", "x"), {"x": T_REAL}, Substitution(), {}, {}, [].append)
        assert t == T_REAL

        # for a vector variable returns TTensor type
        vec3 =  TTensor(((3, "invariant"),))
        t, _ = infer_expr(("var", "v"), {"v": vec3}, Substitution(), {}, {}, [].append)
        assert t == vec3

        # Unknown variable returns ``None``"""
        errors = []
        t, _ = infer_expr(("var", "z"), {}, Substitution(), {}, {}, errors.append)
        assert t is None
        assert errors == []

    def test_imaginary_node(self):
        """``("imaginary",)`` returns ``T_COMPLEX``."""
        t, _ = infer_expr(("imaginary",), {}, Substitution(), {}, {}, [].append)
        assert t == T_COMPLEX
    ...
    def test_slice_oob_reports_error(self):
        ...
        assert len(errors) >= 1
        assert errors[0] == "Slice end 10 is out of bounds for 'a' with leading dimension 5"
    ...
    def test_unknown_tag_reports_error(self):
        ...
        errors = []
        t, _ = infer_expr(("invalid_tag", 99), {}, Substitution(), {}, {}, errors.append)
        assert t is None
        assert errors == ["Unknown expression type: invalid_tag"]

        # second unknown tag produces the correct error message
        infer_expr(("mystery_op", "a", "b"), {}, Substitution(), {}, {}, errors.append)
        assert len(errors) == 2
