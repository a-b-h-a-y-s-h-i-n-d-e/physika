...
Where :math:`T` is temperature, :math:`x` is space, :math:`t` is time, and
...
.. note::
   ``visualize_results`` is not a built-in Physika function. To use it,
   add the following helper to ``physika/runtime.py``:

   .. code-block:: python

      def visualize_results(true_values, pred_values):
