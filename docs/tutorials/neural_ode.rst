...
In this tutorial we will implement a simple Neural ODE model and train it to predict dynamics of damped oscillator. [Chen_NeuralODE]_
...
Now lets create a function to generated dataset:
...
.. code-block:: text
    ...
    def adjoint_solver(pred_traj: ℝ[m, 2, 1], true_trajectory: ℝ[m, 2, 1], Δt: ℝ, n_steps: ℕ, model: ODEFunc): list:
        ...
        for i:ℕ(n_steps - 1):
            ...
            idx = n_steps - 2 - i
            aug_state[1] = aug_state[1] + 2 * (pred_traj[idx] - true_trajectory[idx])
